...
function PaperFile() {
              ...
              {record?.FileStatus !== 4 && (
                <Tooltip title="Xem chi tiết và sửa hồ sơ" color="#2a2a2a" mouseLeaveDelay={0}>
                  <Button
                    ...
                    onClick={() => {
                      history.push(`paper-file/${record?.ObjectGuid}`)
                    }}
                  />
                </Tooltip>
              )}
                ...
                <Tooltip title="Danh sách tài liệu" color="#2a2a2a" mouseLeaveDelay={0}>
                  <Badge count={null} size="small">
                    <Button
                      ...
                      onClick={() => history.push(`/paper-file/${record.ObjectGuid}/paper`)}
                    />
                  </Badge>
                </Tooltip>
              )}
  ...
  const onReceiveFile = () => {
    ...
    FileService.receive({
      ...
      .then(res => {
        ...
        getListPaperFile(conditionSearch)
      })
      .finally(() => setIsLoading(false))
  }
  ...
  const handleChangeEasySearch = value => {
    getListPaperFile({ ...conditionSearch, TextSearch: value })
  }

  const handleChangeAdvanceSearch = allValues => {
    getListPaperFile({ ...conditionSearch, ...allValues })
  }
  ...
  const checkListReceiveReject = (receive = true) => {
    if (!listFile.length || !listReject.length) {
      toast({
        title: `${
  ...
  return (
    <DataManagementWrapper>
      <BreadcrumbWrapper>
        ...
        <Breadcrumb.Item onClick={() => history.push(`/paper-file`)}>
          <a>Danh sách hồ sơ</a>
        </Breadcrumb.Item>
      </BreadcrumbWrapper>
      ...
      <TableHeadingWrapper>
        <div>
          <div className="table-heading">Danh sách hồ sơ giấy</div>
        ...
        <Space>
          <ButtonCustom
            ...
            size={15}
            onClick={() => history.push(`/paper-file/1/create`)}
          />
          <ButtonCustom
            text="Tiếp nhận"
            color="var(--color-primary)"
